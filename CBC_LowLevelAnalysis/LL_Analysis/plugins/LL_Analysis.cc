...
void LL_Analysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   std::vector<int> hits_dut_A;
   std::vector<int> hits_dut_B;
   std::vector<int> hits_fix_A;
   std::vector<int> hits_fix_B;
	   ...
	   for(; DSiter != DSViter->data.end(); DSiter++) // hit loop
	   {
	       int detid = DSViter->id;
  		   ...
  		   if (adc > 250)
  		   {
  			   ...
  			   //DUT Chips A & B
  	           if ((detid == 51001 || detid == 51002) && DSiter->row() < 127) hits_dut_A.push_back(DSiter->row());
   	           if ((detid == 51001 || detid == 51002) && DSiter->row() >= 127) hits_dut_B.push_back(DSiter->row());
   
   			   //FIX Chips A & B
   	           if ((detid == 51011 || detid == 51012) && DSiter->row() < 127) hits_fix_A.push_back(DSiter->row());
   	           if ((detid == 51011 || detid == 51012) && DSiter->row() >= 127) hits_fix_B.push_back(DSiter->row());
  		   }
	   } //End of hit loop
   } //End of module loop
   ...
   // h_n_hits_dut_A->Fill(static_cast<int>(hits_dut_A.size()));
//    h_n_hits_dut_B->Fill(static_cast<int>(hits_dut_B.size()));
//    h_n_hits_fix_A->Fill(static_cast<int>(hits_fix_A.size()));
//    h_n_hits_fix_B->Fill(static_cast<int>(hits_fix_B.size()));
...
LL_Analysis::beginJob()
{
    ...
    h_hits_FIX_t = fs->make<TH1D>("h_hits_FIX_t","Hits FIX bot",256,0.,256.);
  	...
  	//Efficiency originally TDC
    // h_eff_dut_b = fs->make<TH1D>("h_eff_dt","dut top",10,0.,10.);
//     h_eff_dut_t = fs->make<TH1D>("h_eff_db","dut bottom",10,0.,10.);
//     h_neff_dut_b = fs->make<TH1D>("h_neff_dt","dut top",10,0.,10.);
//     h_neff_dut_t = fs->make<TH1D>("h_neff_db","dut bottom",10,0.,10.);
  	...
  	h_n_hits_fix_A = fs->make<TH1D>("h_n_hits_fix_B","Number of Hits FIX chip B", 256,0.,256.);
}
